...
# ALPACA_API_KEY = os.getenv("VIX_ALPACA_API_KEY","")
# ALPACA_API_SECRET = os.getenv("VIX_ALPACA_API_SECRET","")
...
HEADERS = {
    "APCA-API-KEY-ID": VIX_ALPACA_API_KEY,
    "APCA-API-SECRET-KEY": VIX_ALPACA_API_SECRET
}

def get_vix_orders(status="all"):
    """Fetch all orders (open, closed, or canceled)."""
    url = f"{ALPACA_BASE_URL}/v2/orders"
    response = requests.get(url, headers=HEADERS, params={"status": status})
    return response.json() if response.status_code == 200 else None

def get_vix_positions():
...
def get_vix_daily_pnl():
    """Fetch PnL details from Alpaca API."""
    account_info = get_vix_account()
    if account_info:
        return {
...
def get_vix_historical_pnl(start_date=None, end_date=None, timeframe="1D"):
...
def get_vix_incremental_pnl(start_date=None, end_date=None, timeframe="1D"):
